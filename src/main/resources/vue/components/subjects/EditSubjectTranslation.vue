<template>
  <div v-if="isOpened" class="mt-5 mb-5">
    <h5>Edit subject translation</h5>
    <hr />
    <b-form-select
...
export default class EditSubjectTranslation extends Vue {
  ...
  mounted() {
    let _this = this;
    EventBus.$on('show-edit-subject-translation-form', function (item) {
      _this.isOpened = true;
      _this.form.translation.value = item.translation;
      _this.selectedSubject = item;
    ...
    EventBus.$on('hide-edit-subject-translation-form', function () {
      _this.isOpened = false;
    });

    EventBus.$on('subject-translation-was-removed', function () {
  ...
  getTranslation() {
    ...
    SubjectTranslationService.getTranslation(subjectId, localeId)
      .then((response: any) => {
  ...
  addTranslation() {
    ...
    SubjectTranslationService.addTranslation(
    ...
    .then(response => {
      ...
      this.$bvToast.toast(`Subject translation was added successfully!`, {
        title: 'Notification',
        autoHideDelay: 5000,
        appendToast: false
      });
  ...
  editTranslation() {
    ...
    SubjectTranslationService.editTranslation(
      this.selectedSubject.id,
      this.selectedLocale,
      this.form.translation.value
    )
    .then(response => {
      EventBus.$emit('subject-translation-was-changed', subjectTranslation);
      this.$bvToast.toast(`Subject translation was edited successfully!`, {
        title: 'Notification',
        autoHideDelay: 5000,
        appendToast: false
      });
  ...
  removeTranslation() {
    EventBus.$emit('show-remove-subject-translation-modal', this.selectedSubject, this.selectedLocale);
  }
}
</script>